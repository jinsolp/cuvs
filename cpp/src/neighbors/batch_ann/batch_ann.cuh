...
// Loads the data in batches onto device for efficiency
//
template <typename T, typename IdxT>
...
//
// global_nearest_cluster [num_rows X n_nearest_clusters] : top n_nearest_clusters closest clusters
// for each data point inverted_indices [num_rows x n_nearest_clusters sized vector] : vector for
// data indices for each cluster cluster_size [n_cluster] : cluster size for each cluster offset
// [n_cluster] : offset in inverted_indices for each cluster
//
...
void build(const raft::resources& handle,
           ...
           batch_ann::index<IdxT, T>& index)
{
  // TODO add data support only float warning
  auto start      = raft::curTimeMillis();
  size_t num_rows = static_cast<size_t>(dataset.extent(0));
  size_t num_cols = static_cast<size_t>(dataset.extent(1));
